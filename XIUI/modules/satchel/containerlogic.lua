...
function containerlogic.build_slot_data(satchel)
    ...
    local function is_container_enabled(container_id, max_slots)
        ...
        -- Prefer the server's usable flag when we've seen ITEM_MAX.
        local usable = packet_container_usable(cid)
        if usable ~= nil then
            return usable and total > 0
        end

        -- No packet yet: Safe2 size is unreliable, so hide it until authoritative;
        -- fall back to slot sampling for everything else.
        if cid == 9 then
            return false
        end
